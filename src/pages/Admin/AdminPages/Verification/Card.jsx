    ...
    <section className="px-8 pt-4.75 pb-10 font-sans">
      <div className="grid grid-cols-1 md:grid-cols-2  gap-10 w-full lg:w-[86%]">
        {cards.map((c, index) => (
          ...
          >
            <h3 className="text-base font-semibold text-[#171C27] pb-4">
              {c.name}
            </h3>
            <div className=" flex flex-col gap-3 justify-center items-center">
              <img src={c.img} alt="" className="min-h-[195px]" />
              <p
                className={`${typeof c.status === "string" && c.status.includes("Failed") ? "text-[#EE606B] bg-[#FFEFF4] px-2.5 py-0.5 rounded-[16px]" : "text-[#027A48] bg-[#ECFDF3] px-2.5 py-0.5 rounded-[16px]"}`}
              >
                {c.status}
              </p>
            </div>
        ...
        <div className="border border-[#E4E7EC] px-5 md:px-15 py-4.25 rounded-xl">
          ...
          <div className="flex flex-col gap-5 justify-center items-center pt-3">
            <button className="py-2.5 px-6 md:px-18 bg-[#407BFF] text-white font-medium rounded-lg cursor-pointer">
              Approve Verification
            </button>
            <button className=" px-5.75 md:px-17.25 py-2.5 text-[#374151] bg-[#F3F4F6] font-medium rounded-lg cursor-pointer">
              Request New ID Card
            </button>
            <button className="md:px-13.75 px-2 py-2.5 text-[#374151] bg-[#F3F4F6] font-medium rounded-lg cursor-pointer">
              Download All Documents
            </button>
            <button className="md:px-19 px-7.75 py-2.5 border border-[#E41D1D] text-[#EF4444] font-medium rounded-lg cursor-pointer">
              Reject Verification
            </button>
          </div>
        </div>
