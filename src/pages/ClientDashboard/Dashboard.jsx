        ...
        <div className="pt-6 pb-4">
          <HeaderSection>
            <div className=" flex gap-4 items-center">
              <div className="flex gap-2.5 md:gap-5 items-center">
                <img src="/home (1).svg" alt="" className="w-3 h-3 md:w-5 md:h-5"/>
                <span className="text-white text-xs md:text-base">Dashboard</span>
              </div>
              <div className="flex gap-2.5 md:gap-5 items-center">
                <img src="/Vector (29).svg" alt=""  className="w-3 h-3 md:w-5 md:h-5"/>
                <span className="text-white text-xs md:text-base">Client Management</span>
              </div>
            </div>
          </HeaderSection>
          <section class="flex flex-col items-center md:items-start lg:flex-row justify-between gap-10 md:gap-6 pt-8">
            <div>
              ...
              <div className="bg-[#45B2C7] flex flex-col md:flex-row items-center gap-8 rounded-[16px] px-[15px] py-[17px] md:px-[27px] md:py-[25px] mt-5 md:mt-10">
                <div>
                ...
                <button className="bg-white text-[#45B2C7] py-2 px-6 rounded-lg cursor-pointer">
                  Add New Client
                </button>
              </div>
            </div>
            <div>
              <div className="grid grid-cols-1 w-[270px] md:w-full  md:grid-cols-3 gap-6 md:gap-4 ">
                {stats.map((stat, index) => (
                  <div key={index} className="bg-white  px-4 py-13 rounded-[12px]">
                    <div className="">
                      <img
                        src={stat.icon}
                        alt={stat.title}
                      ...
                      {stat.value}
                        ...
                        <div className="bg-[#E7F6EC] rounded-[10px] px-0.5 flex gap-0.5 items-center text-xs font-medium">
                           ...
                           <span> {stat.perc} </span>
                            ...
                            <span className="text-[10px] text-[#7A7A9D]">{ stat.change}</span>
                            </div>
                  </div>
                ))}
              </div>
